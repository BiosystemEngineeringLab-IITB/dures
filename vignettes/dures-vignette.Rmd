...
```
We will also take a look at the size for the first and the third spectra belonging to the features ID `1982` and `872` respectively. We performed intra-spectrum grouping, meaning, fragments within a default tolerance of 0.05 Da of one another were merged (mean of m/z and sum of intensities). The resulting spectra are stored in the folder `MS2_scans_before_denoising`. Due the intra-spectra grouping, we can clearly see a reduction in the number of peaks. But before that, we need to check the identity of the spectra to make sure we are comparing the same spectra before and after step 2 of the analysis.
...
For the first feature, `1982`, we will consider the spectrum titled `20092020_CovidMild_P294_P_2.mzML_scan_1587` - third on the list above. Let's look at the number of fragments before step 2.

```{r peaksdata_before_intra}
peaksData(l2$sps_top_tic_2[[1]][3])[[1]]
```
There are $23$ fragments before intra-spectrum grouping. Now, if I look at the same spectrum from the folder `MS2_scans_before_denoising` (subfolder name: $1982$ and text file name: `20092020_CovidMild_P294_P_2.mzML_scan_1587.txt`), there are $22$ fragments. The fragments with mz $304.11557$ and $304.15106$ were combined into one fragment after intra-spectrum grouping.  
...
Similar observations were made for the metabolite ID `872` using the spectrum `13092020_CovidMild_P282_P_2.mzML_scan_1120`. We will look at the original number of fragments before grouping and then look at the corresponding entry in the folder `MS2_scans_before_denoising`. Before and after grouping, the number of fragments reduced from $98$ to $81$, respectively. 
