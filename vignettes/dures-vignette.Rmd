...
- `test_1` folder contains an folder named `mzml_files\`, and two files titled `Sample-and-feature-wise-RT-tolerance.txt` and `feature_list.txt`. There are three columns in the `feature_list.txt` file, "ID", "mz" and "RT". The additional file `Sample-and-feature-wise-RT-tolerance.txt` contain the sample-wise RT tolerances for all features. The number of rows of both files are the same while the `Sample-and-feature-wise-RT-tolerance.txt` file has twice as many columns as `feature_list.txt`, with `RT_min_` and `RT_max` is appended to every sample name.
